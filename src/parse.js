  ...
  getData (){
    ...
    if(doctorList.length == 0) {
      ...
      for(let i = 0; i < doctorList.length; i++) {
        ...
        $("#doctor-info").append(`<p id="doctor-name">${this.doctorName}</p>`);
        this.specialty = doctorList[i].specialties[0].actor;
        $("#doctor-info").append(`<p>Specialty: ${this.specialty}</p>`);
        this.bio = doctorList[i].profile.bio;
        $("#doctor-info").append(`<p>Biography: ${this.bio}</p>`);
        ...
        for(let j = 0; j < (doctorList[i].practices).length; j++){
          ...
          this.acceptNew ? $("#doctor-info").append(`<p>Accept New Patients: Yes</p>`) : $("#doctor-info").append(`<p>Accept New Patients: No</p>`);

          this.officeName = listPractice.name;
          this.website = listPractice.website;
          $("#doctor-info").append(`<p>Website: <a href=${this.website}>${this.officeName}</a></p>`);
          ...
          $("#doctor-info").append(`<p>Address: ${this.officeAddress}</p>`);
          $("#doctor-info").append(`<p>TEL: ${this.phoneNumber}</p><hr>`);
          break;
        }
      }
    }  
